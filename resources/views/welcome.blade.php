...
<nav class="fixed top-0 inset-x-0 z-[100] transition-all duration-300 bg-white/70 dark:bg-[#050505]/70 backdrop-blur-2xl border-b border-black/5 dark:border-white/[0.05]">
    <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
            ...
            @auth
                <a href="{{ route('dashboard') }}" class="hidden sm:block text-[13px] font-semibold text-zinc-600 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white transition-colors">Dashboard</a>
                <a href="{{ route('dashboard') }}" class="inline-flex items-center justify-center px-4 py-2 rounded-full bg-zinc-900 dark:bg-white text-white dark:text-black text-[13px] font-bold hover:scale-105 transition-transform shadow-lg shadow-black/10 dark:shadow-white/10">Go to Cockpit &rarr;</a>
            @else
                <a href="{{ route('login') }}" class="hidden sm:block text-[13px] font-semibold text-zinc-600 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white transition-colors">Log in</a>
                <a href="{{ route('register') }}" class="inline-flex items-center justify-center px-4 py-2 rounded-full bg-zinc-900 dark:bg-white text-white dark:text-black text-[13px] font-bold hover:scale-105 transition-transform shadow-lg shadow-black/10 dark:shadow-white/10">Sign up</a>
            @endauth
        </div>
    </div>
</nav>
...
<section class="pt-40 pb-24 relative z-10 flex flex-col items-center justify-center min-h-[90vh]">
    <div class="max-w-[1000px] mx-auto px-6 text-center">
        ...
        <div class="inline-flex items-center gap-2.5 px-3 py-1.5 rounded-full bg-white/60 dark:bg-white/5 border border-black/5 dark:border-white/10 backdrop-blur-md mb-8 shadow-sm">
            ...
            <span class="text-[12px] font-semibold text-zinc-800 dark:text-zinc-300 tracking-wide">{{ count($mappedFlights) }} Pilots Online Right Now</span>
        ...
        <p class="text-lg md:text-xl text-zinc-600 dark:text-zinc-400 max-w-2xl mx-auto leading-relaxed mb-10 font-medium">
            Over <strong class="text-zinc-900 dark:text-white">{{ number_format($scheduleCount) }}</strong> real-world routes, advanced ACARS tracking, and an immersive career progression platform.
        </p>

        {{-- CTAs --}}
        <div class="flex flex-col sm:flex-row items-center justify-center gap-4 mb-20">
            @auth
                <a href="{{ route('flights') }}" class="w-full sm:w-auto inline-flex items-center justify-center px-8 py-3.5 rounded-full bg-red-600 hover:bg-red-700 text-white font-bold text-sm transition-all hover:shadow-[0_0_20px_rgba(220,38,38,0.4)] hover:-translate-y-1">Find a Flight</a>
            @else
                <a href="{{ route('register') }}" class="w-full sm:w-auto inline-flex items-center justify-center px-8 py-3.5 rounded-full bg-red-600 hover:bg-red-700 text-white font-bold text-sm transition-all hover:shadow-[0_0_20px_rgba(220,38,38,0.4)] hover:-translate-y-1">Create free account</a>
            @endauth
        ...
        <div class="relative bg-white/50 dark:bg-black/50 backdrop-blur-2xl border border-white/50 dark:border-white/10 rounded-2xl overflow-hidden shadow-2xl shadow-black/10 dark:shadow-white/5 transform rotate-x-12 hover:rotate-x-0 transition-transform duration-700 ease-out">
            ...
            <div class="flex h-[400px] bg-white/80 dark:bg-zinc-950/80">
                ...
                <div class="w-48 border-r border-black/5 dark:border-white/5 p-4 flex flex-col gap-1">
                    ...
                    @foreach(['Dashboard','Bookings','Live Map','Fleet'] as $i => $item)
                        <div class="px-3 py-2 rounded-lg text-xs font-semibold {{ $i===0 ? 'bg-red-500/10 text-red-600 dark:text-red-500' : 'text-zinc-600 dark:text-zinc-400' }} flex items-center gap-3">
                            <div class="w-4 h-4 rounded {{ $i===0 ? 'bg-red-500' : 'bg-zinc-300 dark:bg-zinc-800' }}"></div>
                            {{ $item }}
                        </div>
                    @endforeach
                </div>
    ...
    <div class="max-w-7xl mx-auto px-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-white/60 dark:bg-zinc-900/40 backdrop-blur-xl border border-black/5 dark:border-white/10 rounded-3xl p-8 hover:-translate-y-2 transition-transform duration-300 shadow-xl shadow-black/5">
                ...
                <p class="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed font-medium">Access over <strong class="text-zinc-900 dark:text-zinc-200">{{ number_format($scheduleCount) }}</strong> real-world schedules. From short regional hops to ultra-long-haul intercontinental routes, fly the fleet you love.</p>
            </div>
...
<section id="live-map" class="py-24 relative z-10 border-y border-black/5 dark:border-white/5 bg-zinc-50/50 dark:bg-black/20 backdrop-blur-3xl">
    <div class="max-w-7xl mx-auto px-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-end mb-10 gap-6">
            ...
            <a href="{{ route('live-map') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white dark:bg-white/10 border border-black/10 dark:border-white/10 text-zinc-900 dark:text-white font-bold text-xs hover:scale-105 transition-transform shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 h-[500px]">
            ...
            <div class="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-black/5 dark:border-white/10 rounded-3xl flex flex-col overflow-hidden shadow-xl shadow-black/5">
                ...
                <div class="flex-1 overflow-y-auto p-2">
                    ...
                    <div onclick="focusFlight({{ $f['id'] }})" class="p-4 rounded-2xl hover:bg-zinc-50 dark:hover:bg-white/5 cursor-pointer transition-colors border border-transparent hover:border-black/5 dark:hover:border-white/5">
                        <div class="flex justify-between items-center mb-3">
                            <span class="font-mono font-bold text-zinc-900 dark:text-white text-sm">{{ $f['flight_number'] }}</span>
                            <span class="text-[10px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">{{ $f['phase'] }}</span>
                        </div>
                        
                        <div class="flex items-center gap-3 mb-3">
                            <span class="text-xl font-black tracking-tighter text-zinc-900 dark:text-white">{{ $f['departure'] }}</span>
                            ...
                            <span class="text-xl font-black tracking-tighter text-zinc-900 dark:text-white">{{ $f['arrival'] }}</span>
                        </div>

                        <div class="flex justify-between items-center font-mono text-[11px] text-zinc-500 dark:text-zinc-400">
                            <span>&#x2191; {{ $f['altitude'] > 0 ? 'FL'.intdiv($f['altitude'],100) : 'SFC' }}</span>
                            <span>&#x2192; {{ $f['ground_speed'] }}kts</span>
                            <span>👨‍✈️ {{ $f['pilot_name'] }}</span>
                        </div>
                    </div>
                    @empty
    ...
    <div class="max-w-7xl mx-auto px-6">
        <div class="bg-zinc-900 dark:bg-white/5 rounded-[2.5rem] p-10 md:p-16 relative overflow-hidden shadow-2xl shadow-black/20">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 relative z-10">
                ...
                <div class="text-center lg:text-left {{ $i < 3 ? 'lg:border-r lg:border-white/10' : '' }}">
                    <div class="text-sm font-bold tracking-widest text-zinc-400 dark:text-zinc-500 uppercase mb-2">{{ $s[0] }}</div>
                    <div class="text-5xl font-black tracking-tighter text-white">{{ $s[1] }}</div>
                </div>
                @endforeach
            </div>
        </div>
...
<section class="py-32 relative z-10 overflow-hidden">
    <div class="max-w-4xl mx-auto px-6 text-center">
        ...
        <div class="flex flex-col sm:flex-row justify-center gap-4">
            @auth
                <a href="{{ route('dashboard') }}" class="inline-flex items-center justify-center px-10 py-4 rounded-full bg-red-600 hover:bg-red-700 text-white font-bold text-base transition-transform hover:scale-105 shadow-lg shadow-red-600/30">Head to Dashboard</a>
            @else
                <a href="{{ route('register') }}" class="inline-flex items-center justify-center px-10 py-4 rounded-full bg-red-600 hover:bg-red-700 text-white font-bold text-base transition-transform hover:scale-105 shadow-lg shadow-red-600/30">Start your career now</a>
            @endauth
        </div>
    </div>
</section>
...
<footer class="border-t border-black/5 dark:border-white/5 py-12 relative z-10">
    <div class="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <p class="text-xs font-semibold text-zinc-500">&copy; {{ date('Y') }} FlyAway Virtual Airline. For simulation purposes only.</p>
    </div>
</footer>
