...
new class extends Component {
    ...
    {
        $query = User::with('rank')->orderBy('created_at', 'desc');
        if ($this->showPending) {
        ...
        $this->pilots = $query->get();
    }

    public function regenerateApiKey($id): void
    {
        $user = User::findOrFail($id);
        $newKey = 'fly-' . bin2hex(random_bytes(16));
        $user->update(['api_key' => $newKey]);
        ActivityLogger::log('regenerate_api_key', 'user', $id, "Regenerated API Key for pilot {$user->name} ({$user->pilot_id})");
        $this->loadPilots();
    ...
    public function approve($id): void
    {
        ...
        ActivityLogger::log('approve_pilot', 'user', $id, "Approved pilot {$user->name} ({$user->pilot_id})");
        $this->loadPilots();
        session()->flash('success', 'Pilot approved successfully.');
    }

    public function confirmSuspend($id): void
    {
    ...
    public function suspend(): void
    {
        ...
        ActivityLogger::log('suspend_pilot', 'user', $id, "Suspended pilot {$user->name} ({$user->pilot_id}): {$this->suspendReason}");

        $this->suspendingId = null;
        $this->suspendReason = '';
        $this->loadPilots();
        session()->flash('success', 'Pilot suspended.');
    }
    ...
    public function reactivate($id): void
    {
        ...
        ActivityLogger::log('reactivate_pilot', 'user', $id, "Reactivated pilot {$user->name} ({$user->pilot_id})");
        $this->loadPilots();
        session()->flash('success', 'Pilot reactivated.');
    }

    public function filterPending(): void
    {
...
<div class="max-w-7xl mx-auto space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ count($pilots) }} pilots</p>
        </div>
        <div class="flex items-center gap-2">
            <input wire:model.live.debounce="search" placeholder="Search pilots..." class="input-field text-sm px-3 py-1.5 w-48">
            <a href="{{ route('admin.export.pilots.csv') }}" class="btn-secondary text-sm">
            ...
            <a href="{{ route('admin.export.pilots.print') }}" target="_blank" class="btn-secondary text-sm">
    ...
    <div x-data="{ show: @entangle('suspendingId') }" x-show="show" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" @click.self="$wire.cancelSuspend()">
                        ...
                        <tr class="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/30">
                            <td class="p-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-lg bg-crimson-100 dark:bg-crimson-900/30 flex items-center justify-center">
                                        <span class="text-xs font-bold text-crimson-700 dark:text-crimson-400">{{ substr($pilot->name, 0, 2) }}</span>
                                    </div>
                                    <div>
                                        <p class="font-medium text-slate-900 dark:text-white">{{ $pilot->name }}</p>
                                        <p class="text-xs text-slate-500">{{ $pilot->email }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="p-4 text-slate-500">{{ $pilot->pilot_id ?? '—' }}</td>
                            <td class="p-4">
                                <span class="badge-info">{{ $pilot->rank?->name ?? 'Candidate' }}</span>
                            </td>
                            <td class="p-4">{{ number_format($pilot->total_hours, 1) }}</td>
                            <td class="p-4">{{ $pilot->total_flights }}</td>
                            <td class="p-4">
                                @if($pilot->status === 'active')
                                    <span class="badge-success">Active</span>
                                @elseif($pilot->status === 'pending')
                                    <span class="badge-warning">Pending</span>
                                @else
                                    <span class="badge-danger">Suspended</span>
                                @endif
                                @if($pilot->suspension_reason)
                                    <div class="text-xs text-red-500 mt-1 max-w-40 truncate" title="{{ $pilot->suspension_reason }}">{{ $pilot->suspension_reason }}</div>
                                @endif
                            </td>
                            <td class="p-4">
                                <div class="flex items-center gap-2 font-mono text-xs text-slate-500">
                                    <span class="max-w-[80px] truncate">{{ $pilot->api_key ? substr($pilot->api_key, 0, 7) . '...' : '—' }}</span>
                                    <button wire:click="regenerateApiKey({{ $pilot->id }})" class="text-crimson-600 hover:text-crimson-800 dark:text-crimson-400" title="Regenerate API Key">
                            ...
                            <td class="p-4 text-right">
                                @if($pilot->status === 'pending')
                                    <button wire:click="approve({{ $pilot->id }})" class="text-sm text-emerald-600 dark:text-emerald-400 hover:underline mr-3">Approve</button>
                                @endif
                                @if($pilot->status === 'active')
                                    <button wire:click="confirmSuspend({{ $pilot->id }})" class="text-sm text-red-600 dark:text-red-400 hover:underline">Suspend</button>
                                @endif
                                @if($pilot->status === 'suspended')
                                    <button wire:click="reactivate({{ $pilot->id }})" class="text-sm text-emerald-600 dark:text-emerald-400 hover:underline">Reactivate</button>
                                @endif
                            </td>
                        </tr>
                    @endforeach
