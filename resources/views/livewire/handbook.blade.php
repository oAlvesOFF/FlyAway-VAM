...
new class extends Component {
    public string $section = 'getting-started';
}; ?>
    ...
    <div class="flex flex-wrap gap-2">
        @foreach([
            ...
            <button wire:click="$set('section', '{{ $key }}')"
                class="px-4 py-1.5 rounded-xl text-sm font-medium transition-all duration-150 {{ $section === $key ? 'bg-crimson-600 text-white shadow-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700' }}">
                {{ $label }}
            </button>
        @endforeach
    </div>
